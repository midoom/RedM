...
Citizen.CreateThread(function() 
	while true do 
		...
		if tonumber(roundtemp) <= -8 then
			TriggerEvent("vorp:NotifyLeft", "~t7~ Temperature : Environ -8°","Vous etes geles couvre-toi, sinon c est la fin!", "rpg_textures", "rpg_cold", 4000)
			elseif tonumber(roundtemp) <= -6 then 
			TriggerEvent("vorp:NotifyLeft", "~pa~ Temperature : Environ -6°","Vous avez froid, tu devrais te couvrir !", "rpg_textures", "rpg_cold", 4000)
		elseif tonumber(roundtemp) <= -4 then 
			TriggerEvent("vorp:NotifyLeft", "~t3~Temperature : Environ -4°","Vous avez tres froid, vous devriez vous couvrir davantage!", "rpg_textures", "rpg_cold", 4000)
		--elseif tonumber(roundtemp) <= 20 then 
			--TriggerEvent("vorp:NotifyLeft", "~o~Temperature : Environ 20°", "Vous avez chaud, etanchez souvent votre soif !", "rpg_textures", "rpg_hot", 3000)
		--elseif tonumber(roundtemp) <= 25 then 
			TriggerEvent("vorp:NotifyLeft", "~d~Temperature : Environ 25°","Vous avez chaud, cherchez un abri et etanchez souvent votre soif!", "rpg_textures", "rpg_hot", 3000)
		elseif tonumber(roundtemp) <= 30 then 
			TriggerEvent("vorp:NotifyLeft", "~t2~Temperature : Environ 30°","Vous avez trop chaud, cherche un abri, sinon c est la fin!", "rpg_textures", "rpg_hot", 3000)
		end
