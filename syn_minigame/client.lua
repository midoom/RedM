...
function openGui(sentLength,taskID,namesent,chancesent,skillGapSent)
    TriggerEvent("vorp:TipBottom", "Appuyer sur E dans le Spot", 4000)
    Wait(1000)
    guiEnabled = true
...
function FactorFunction(pos)
    if not pos then
        ...
        if factor == 0.5 and calm then
            calm = false
            TriggerEvent("notification","Vous etes frustre", 2)
        end
        TriggerEvent("factor:restore")        
    else
        if factor > 1.0 or factor == 0.9 then
            if not calm then
                TriggerEvent("notification","Vous etes de nouveau calme")
                calm = true
            end            
            factor = 1.0
            return
