...
AddEventHandler("seed:used", function()
    TriggerEvent("vorp:TipBottom", "Vous ne pouvez pas utiliser plus dune graine", 5000)
end)
...
AddEventHandler('poke_planting:fin2', function(object2, x, y, z, key, hash1, hash2, hash3)
    ...
    TriggerEvent("vorp:TipRight", "Votre plante a muri!", 4000)
...
function harvestPlant(key)
    ...
    TriggerEvent("vorp:TipRight", "Recolte...", 10000)
    Wait(10000)
		...
		if myPlants[1] ~= nil then
			...
			for k, v in ipairs(myPlants) do
				if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 25.0 then
					if v.stage == 1 then
						DrawText3D(v.x, v.y, v.z, 'Arroser la plante')
					end
					if v.stage == 2 then
						DrawText3D(v.x, v.y, v.z, 'Croissance: ' .. v.timer)
					end
					if v.stage == 3 then
						DrawText3D(v.x, v.y, v.z, 'Appuyer sur [~e~B~q~] pour Recolter!')
					end
					if v.prompt then
...
function animacion2()
    ...
    TriggerEvent("vorp:TipRight", "Arroser la plante...", 7000)
    TriggerEvent("vorp_inventory:CloseInv")
    Wait(7000)
    ClearPedTasksImmediately(PlayerPedId())
end
