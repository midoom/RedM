...
function closeBattlepass() {
    $.post("http://AW_Battlepass/closeNUI", JSON.stringify({}));
}
...
$(function() {
		...
		if (item.type == "enable_battlepass") {
			...
			$("#left-side").append(
				...
				`</div>`+
					...
					`<span id="level_progress_perc">Livello: `+ item.level +` |</span> <span id="level_progress_perc">`+ item.percentage +`%</span>`+
			...
			$("#packdisplay").append(
				`<div id="packDisplayBox">`+
					...
					`<div>`+
					   `<div  id="pack_display_level_box_description_title">★ Livello: ` + item.level + ` ` + `Ricompensa</div>`+
					...
					`<i class="fa fa-gift"> RICEVI LA TUA RICOMPENSA</i></i>`+
				    `</div>`+
					...
					`<div id="levelBox">`+
						...
						`<div id=" ">ㅤ</div>`+
							...
							`<div>`+
								...
								`<span id="level_box_reward_cost"> Il costo è di: ` + item.cost + ` Dobloni </span>  <br>`+
							`</div>`+
							...
							`<div id="level_box_canBuy" level='`+ levelscheck +`'>`+
							    `<i class="fas fa-coins"> ACQUISTA </i>`+
							`</div>`+

						`</div>`+
						`<div id=" ">ㅤ</div>`+
						`<div id="level_counter">LIVELLO `+ levelscheck+`</div>`+
					`</div>`
				);
			}
				...
				$("#levelpacks").append(
					`<div id="levelBox">`+
						...
						`<div id=" ">ㅤ</div>`+
							...
							`<div>`+
								`<span id="level_box_reward"> `+ level_detail.title +` </span>  <br>`+
								`<span id="level_box_reward_cost">Nessun Costo</span>  <br>`+
							`</div>`+
							`<div id="level_box_claimer" level='`+ levelscheck + `' item='` + level_detail.title + `' description='` + level_detail.description + `' image='` + level_detail.image + `' >`+
	...
	$("#levelpacks").on("click", "#level_box_canBuy", function() {
		...
		$.post('http://AW_Battlepass/buyLevel', JSON.stringify({
	...
	$("#packdisplay").on("click", "#level_box_display_claim", function() {
		...
		$.post('http://AW_Battlepass/claimRewards', JSON.stringify({
			level : $level
		}))
		return
	...
	$("#levelpacks").on("click", "#level_box_canClaim", function() {
		...
		$.post('http://AW_Battlepass/displayReceivedReward', JSON.stringify({
			item : $item,
	...
	$("#levelpacks").on("click", "#level_box_claimer", function() {
		...
		$.post('http://AW_Battlepass/displayReward', JSON.stringify({
			item : $item,
			description : $description,
			level : $level,
			image : $image
