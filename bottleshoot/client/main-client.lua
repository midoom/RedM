...
local config = {
	...
	language = {
		['en'] = {
			["prompt_press"] = 'Appuyer',
			["prompt_play"] = 'Jouer',
			["prompt_end"] = 'Terminer',
			["prompt_game_name"] = 'Bottle Game',
			["wrong_weapon"] = "Mauvaise arme",
			["goto_spot"] = 'Dirigez-vous vers le lieu!',
			["game_hits"] = 'Hits',
			["game_misses"] = 'Ratés',
			["game_hit"] = 'Hit!',
			["game_miss"] = 'Manquer!',
			["game_shoot"] = 'Shoot les bouteilles!',
...
function createCrate(crate) 
	...
	while not HasModelLoaded(crate_model) do
		Citizen.Wait(1)
		print('Waiting for model load... ('..crate_model..')')
	end
...
function createBottle(bottle, handle_thread)
	...
	while not HasModelLoaded(bottle_model) do
		Citizen.Wait(1)
		print('Waiting for model load... ('..bottle_model..')')
	end
