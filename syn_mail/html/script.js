...
function loadnewmsgs() { 
    ...
    if (createbutton) {
        $('#list3').append(
            `<button id="newmail"  onclick='viewmsg(${msg})' type="button">Vous avez ${buttoninfo} Lettre non lues</button>`
        );
       
    }
    else if (!createbutton) {
        $('#list3').append(
            `<button id="newmail" type="button">Pas de nouvelle lettre</button>`
        );
    }
...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (data.action == "show") {
            ...
            $('#landingtitle').html('')
            $('#landingtitle').append(
                `<h3 id = "landingtitle">Numero de Boite:${mailingadress}</h3> `
            );
...
$("#submit3").click(function (event) {
    ...
    $("#container-adressbook").hide();  
    $.post('http://syn_mail/send', JSON.stringify({ad:ad,mailingadress:mailingadress, sendto:sendto,msgbody:msgbody, change:change,changered:changered
