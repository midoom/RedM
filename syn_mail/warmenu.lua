...
local menus = { }
local keys = { up = 0x911CB09E, down = 0x4403F97F, left = 0xAD7FCC5B, right = 0x65F9EC5B, select = 0x43DBF61F, back = 0x156F7119}--, back = 0x308588E6
local optionCount = 0
...
local function drawText(str, x, y, font, color, scale, center, shadow, alignRight)
	local str = CreateVarString(10, "LITERAL_STRING", str)


	SetTextScale(scale, scale)
	SetTextColor(math.floor(color.r), math.floor(color.g), math.floor(color.b), math.floor(255))
...
local function drawRect(x, y, width, height, color)
	DrawSprite("menu_textures", "translate_bg_1a", x, y, width, height, 0.8, color.r, color.g, color.b, color.a)
end
...
local function drawTitle()
	if menus[currentMenu] then
		...
		DrawSprite("menu_textures", "translate_bg_1a", x, y, menus[currentMenu].width, titleHeight, 0.8, 000, 2, 2, 200, 1)
		--DrawSprite("generic_textures", "menu_header_1a", x, y, menus[currentMenu].width - 0.45, titleHeight, 0.1, 100, 1, 1, 150, 0)
...
local function drawSubTitle()
	if menus[currentMenu] then
		...
		if optionCount > menus[currentMenu].maxOptionCount then
			drawText(tostring(menus[currentMenu].currentOption)..' / '..tostring(optionCount), menus[currentMenu].x + 0.035, y - buttonHeight / 2 + buttonTextYOffset, buttonFont, subTitleColor, buttonScale, false, false, true)
		end
	end
end
...
local function drawButton(text, subText)
	...
	if multiplier then
		...
		DrawSprite("menu_textures", "translate_bg_1a", x, y, menus[currentMenu].width, buttonHeight, 0.8, 0, 0, 0, 160)
		drawRect(x, y, menus[currentMenu].width, buttonHeight, backgroundColor)
		drawText(text, menus[currentMenu].x + buttonTextXOffset, y - (buttonHeight / 2) + buttonTextYOffset, buttonFont, textColor, buttonScale, true, shadow)

		if subText then
			drawText(subText, menus[currentMenu].x - 0.12, y - buttonHeight / 2 + buttonTextYOffset, buttonFont, subTextColor, buttonScale, false, shadow, true)
		end
	end
end
